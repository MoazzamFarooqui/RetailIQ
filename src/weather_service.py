"""Weather API integration service with Pakistan-specific seasons.

Uses OpenWeatherMap API to fetch live weather data.
Falls back gracefully if API key is not configured.

Pakistan seasons:
- Summer (April-June): Hot and dry
- Monsoon (July-September): Hot and wet
- Autumn/Transition (October): Mild
- Winter (November-February): Cool to cold
- Spring (March): Mild, transition to summer
"""
...
# ── Pakistan city coordinates (default: Lahore) ─────────────────────────────
PAKISTAN_CITIES = {
...
# ── Seasonal product demand mapping for Pakistan ────────────────────────────
SEASONAL_PRODUCT_DEMAND = {
    'Summer': {
        ...
        'low_demand_keywords': [
            'soup', 'coffee', 'hot tea', 'blanket', 'heater',
            'winter', 'jacket', 'sweater', 'gloves',
        ],
        'advice': '☀️ Hot summer — stock up on cold drinks, ice cream, juices, and beverages.',
        'temp_threshold': 35,  # °C — above this is "extreme heat" alert
    },
    'Monsoon': {
        'months': [7, 8, 9],
        'high_demand_keywords': [
        ...
        'low_demand_keywords': [
            'ice cream', 'cold drink', 'sunblock', 'sunglasses',
        ],
    ...
    'Winter': {
        ...
        'low_demand_keywords': [
            'ice cream', 'cold drink', 'mineral water', 'soda',
            'air conditioner', 'fan', 'sunscreen',
        ],
        'advice': '❄️ Cold weather — stock up on tea, coffee, soups, blankets, and winter essentials.',
        'temp_threshold': 10,  # °C — below this is "cold wave" alert
    },
    'Spring': {
        'months': [3],
        'high_demand_keywords': [
            'juice', 'fresh fruit', 'yogurt', 'salad', 'green vegetable',
        ],
        'low_demand_keywords': [],
    ...
    'Autumn': {
        'months': [10],
        'high_demand_keywords': [
            'tea', 'coffee', 'soup', 'dry fruit', 'nuts',
        ],
        'low_demand_keywords': [
...
class WeatherService:
    ...
    def fetch_current_weather(self, city: str = 'Lahore') -> dict:
        ...
        try:
            ...
            # Add Pakistan season
            result['season'] = self.get_season(datetime.now())
    ...
    def fetch_forecast(self, city: str = 'Lahore', days: int = 7) -> pd.DataFrame:
        """Fetch weather forecast for a Pakistan city for the next N days."""
        if not self.enabled:
        ...
        try:
            if city in PAKISTAN_CITIES:
                ...
                url = f'{BASE_URL}/forecast'
                params = {'q': f'{city},PK', 'appid': self.api_key, 'units': 'metric', 'cnt': days * 8}
            ...
            daily.columns = ['date', 'temp_c', 'feels_like_c', 'temp_min_c', 'temp_max_c',
                              'humidity_pct', 'pressure_hpa', 'wind_speed_ms', 'rain_mm',
                              'weather_condition', 'weather_description']

            # Add season for each day
            daily['season'] = daily['date'].apply(
                lambda d: self.get_season(datetime.combine(d, datetime.min.time()))
            )
    ...
    @staticmethod
    def _fallback() -> dict:
        """Return Pakistan-appropriate fallback data when API is unavailable."""
        now = datetime.now()
        season = WeatherService.get_season(now)
        # Provide realistic defaults based on season
        defaults = {
    ...
    def get_seasonal_demand_advice(date=None) -> dict:
        """Get demand advice for the current season with product recommendations."""
        if date is None:
            date = datetime.now()
    ...
    def get_extreme_weather_advice(weather_data: dict) -> str:
        """Get inventory advice based on extreme weather conditions."""
        ...
        # Heat wave
        summer_info = SEASONAL_PRODUCT_DEMAND.get('Summer', {})
        if temp >= summer_info.get('temp_threshold', 40):
            alerts.append(f"🔥 Extreme heat ({temp:.0f}°C) — high demand expected for cold drinks, water, ice cream.")

        # Cold wave
        winter_info = SEASONAL_PRODUCT_DEMAND.get('Winter', {})
        if temp <= winter_info.get('temp_threshold', 10):
            alerts.append(f"❄️ Cold wave ({temp:.0f}°C) — stock up on tea, coffee, soups, and warm clothing.")

        # Heavy rain / monsoon
        if rain > 10 or 'rain' in condition or 'drizzle' in condition:
            alerts.append("🌧️ Heavy rain expected — umbrellas, raincoats, and indoor snacks will see higher demand.")

        # High humidity
        if humidity > 80:
            alerts.append("💧 High humidity — beverages, cold drinks, and dehumidifiers may see increased demand.")
        ...
        return ' | '.join(alerts) if alerts else 'Normal weather conditions — standard demand patterns expected.'
    ...
    def get_seasonality_features(df: pd.DataFrame, date_col: str = 'date') -> pd.DataFrame:
        ...
        # Ensure all 5 seasons are present
    ...
    def add_weather_features(df: pd.DataFrame, weather_df: pd.DataFrame = None) -> pd.DataFrame:
        ...
        If weather_df is provided, merges it. Otherwise creates synthetic
        weather features based on season.
        """
        df = df.copy()
        ...
        if weather_df is not None and len(weather_df) > 0:
            ...
            # Synthetic weather based on Pakistan season
            df['season'] = dates.apply(WeatherService.get_season)
            season_weather = {
    ...
    def get_weather_impact_multiplier(temp_c: float, season: str, condition: str = '') -> float:
        """Get a demand multiplier based on weather conditions.

        Hot weather → cold drink demand up
        Cold weather → hot beverage demand up
        Rain → indoor/snack demand up
        """
        multiplier = 1.0

        # Temperature impact
        if temp_c >= 35:
            multiplier = 1.3  # Heat drives beverage demand
        elif temp_c >= 30:
            multiplier = 1.15
        elif temp_c <= 10:
            multiplier = 1.25  # Cold drives soup/tea demand
        elif temp_c <= 15:
            multiplier = 1.1
...
def get_pakistan_cities() -> dict:
    """Return the list of supported Pakistan cities."""
