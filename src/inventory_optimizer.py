"""Inventory optimization with Pakistan holiday/season pre-stock alerts and demand multipliers."""
...
class InventoryOptimizer:
    ...
    def optimize_inventory_for_item(self, historical_sales, lead_time_days=7, demand_multiplier=1.0):
        """Optimize inventory for a single item with optional demand multiplier."""
    ...
    def generate_inventory_recommendations(self, df, current_stock_col='current_stock',
                                            demand_multiplier=1.0):
        """Generate inventory recommendations for all items with optional demand multiplier."""
        recommendations = []
        ...
        for (item_id, store_id), group in item_store_groups:
            ...
            optimization = self.optimize_inventory_for_item(
                historical_sales, demand_multiplier=demand_multiplier
            )
            ...
            if stockout:
                recommendation['stockout_in_days'] = stockout['days_remaining']
                recommendation['stockout_date'] = stockout['predicted_date']
            else:
                recommendation['stockout_in_days'] = 999
                recommendation['stockout_date'] = 'N/A'
    ...
    def identify_stockout_risk(self, recommendations_df, threshold_days=7):
        ...
        at_risk = at_risk.sort_values('days_of_stock')
        return at_risk
    ...
    def identify_excess_inventory(self, recommendations_df, excess_threshold=2):
        ...
        excess = excess.sort_values('days_of_stock', ascending=False)
        return excess

    def predict_stockout_date(self, current_stock, avg_daily_demand):
        """Predict the date when stock will run out."""
    ...
    @staticmethod
    def get_current_demand_multiplier() -> dict:
        """Get demand multiplier based on current Pakistan season + upcoming holidays.

        Returns a dict with multiplier and explanation.
        """
        ...
        season_multipliers = {
            'Summer': 1.15,   # General increased consumption
            'Monsoon': 1.05,
            'Winter': 1.10,   # Hot beverages, warm clothing
            'Spring': 1.0,
            'Autumn': 1.0,
        }
        ...
        return {
            ...
            'reasons': reasons,
            'season': season,
            'has_pre_holiday': bool(pre_window),
            'pre_holiday_info': pre_window,
        }

    @staticmethod
    def get_holiday_stock_advice() -> list:
        """Get actionable inventory advice for upcoming Pakistan holidays/seasons.

        Returns a list of dicts with timing, product suggestions, and rationale.
        """
        from src.holiday_service import HolidayService
        ...
        upcoming = holiday_svc.get_upcoming_holidays(today, limit=10)

        for _, holiday in upcoming.iterrows():
            h_name = holiday['name']
            h_date = holiday['date']
            days_until = holiday.get('days_until', (h_date - pd.Timestamp(today)).days)
            category = holiday.get('category', holiday_svc.categorize_holiday(h_name))
            advice_text = holiday.get('advice', '')

            if advice_text:
                advice_list.append({
                    'type': 'holiday_stock_up',
                    'holiday': h_name,
                    ...
                    'demand_multiplier': holiday_svc.get_demand_multiplier(h_name),
                })

        return advice_list

    @staticmethod
    def check_seasonal_product_alerts(product_name: str) -> dict:
        """Check if a product has seasonal demand alerts based on its name/keywords."""
        from src.weather_service import WeatherService, SEASONAL_PRODUCT_DEMAND
        ...
        for keyword in season_info.get('high_demand_keywords', []):
            if keyword.lower() in product_lower:
                return {
                    'alert': 'high_demand',
                    'season': season,
                    'message': f"⚠️ **{product_name}** is in high demand during {season}. "
                               f"Increase stock levels to meet expected demand.",
        ...
        for keyword in season_info.get('low_demand_keywords', []):
            if keyword.lower() in product_lower:
                return {
                    'alert': 'low_demand',
                    'season': season,
                    'message': f"ℹ️ **{product_name}** has lower demand during {season}. "
                               f"Reduce stock levels to avoid overstock.",
                    'action': 'Reduce stock',
                }
